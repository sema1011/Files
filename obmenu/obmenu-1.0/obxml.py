...
class ObMenu:
	...
	def _get_item_props(self,node):
		...
		if node.hasAttribute("icon"):
			icn = node.attributes["icon"].nodeValue
		accion = ""
		param = ""
		prmpt = ""
			...
			if it.nodeType == 1:
				...
				if accion.lower() == "execute":
					for itm in it.childNodes:
						if itm.nodeType == 1 and (itm.nodeName.lower() == "execute" or itm.nodeName.lower() == "command"):
							for item in itm.childNodes:
								if item.nodeType == 3:
									param = item.nodeValue.strip()
						elif itm.nodeType == 1 and itm.nodeName.lower() == "prompt":
							for item in itm.childNodes:
								if item.nodeType == 3:
									prmpt = item.nodeValue.strip()
		return { "type": "item", "icon": icn, "label": etiqueta, "action": accion, "execute": param, "prompt": prmpt }
	...
	def _get_menu_props(self, node):
		...
		return { "type": "menu", "icon": icn, "label": lb, "action": act, "execute": ex, "id": mid }

	# get the properties of a separator from the xml, and returns them as a
	# dictionary.
	def _get_separator_props(self, node):
		lb = ""
		if node.hasAttribute("label"):
			lb = node.attributes["label"].nodeValue
		return { "type": "separator", "label": lb, }
	...
	def setItemLabel(self, menu, n, label):
		itm = self._get_dom_item(menu,n)
		itm.attributes["label"].nodeValue = label

	def setItemIcon(self, menu, n, icn):
		itm = self._get_dom_item(menu,n)
		if icn != "" : itm.attributes["icon"] = icn
		elif itm.hasAttribute("icon"): itm.removeAttribute("icon")

	def setItemProps(self, menu, n, action, exe, prmpt):
		itm = self._get_dom_item(menu,n)
				...
				if action == "Execute":
					if not it.childNodes:
						...
						for i in it.childNodes:
							if i.nodeType == 1 and (i.nodeName == "execute" or i.nodeName == "command"):
					...
					if prmpt != "" :
						prm = False
						for i in it.childNodes:
							if i.nodeType == 1 and i.nodeName == "prompt":
								prm = True
								for item in i.childNodes:
									if item.nodeType == 3:
										item.nodeValue = prmpt
						if not prm:
							elm = xml.dom.minidom.Element("prompt")
							txt = xml.dom.minidom.Text()
							txt.nodeValue = prmpt
							elm.appendChild(txt)
							it.appendChild(elm)
	...
	def setSeparatorLabel(self, menu, n, label):
		itm = self._get_dom_item(menu,n)
		if label != "" : itm.attributes["label"] = label
		elif itm.hasAttribute("label"): itm.removeAttribute("label")

	def setMenuLabel(self, menu, label):
		mnu = self._get_dom_menu(menu)
		if mnu: mnu.attributes["label"].nodeValue = label

	def setMenuIcon(self, mid, icon):
		mnu = self._get_dom_menu(mid)
		if mnu:
			if icon != "" : mnu.attributes["icon"] = icon
			elif mnu.hasAttribute("icon"): mnu.removeAttribute("icon")
	...
	def setRefIcon(self, parent, mid, icon):
		prnt = self._get_dom_menu(parent)
		if prnt: mnu = self._get_dom_ref(mid, prnt)
		if mnu:
			if icon != "" : mnu.attributes["icon"] = icon
			...
			if i.nodeType == 1:
				if n == num:
					if i.nodeName == "menu":
						...
						return self._get_separator_props(i)
